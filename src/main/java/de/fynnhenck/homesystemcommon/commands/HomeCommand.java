package de.fynnhenck.homesystemcommon.commands;

import de.fynnhenck.homesystemapi.api.Home;
import de.fynnhenck.homesystemapi.api.HomeSystemDatabase;
import de.fynnhenck.homesystemcommon.config.HomeMessages;
import de.fynnhenck.homesystemcommon.config.HomePermissions;
import org.bukkit.ChatColor;
import org.bukkit.Location;
import org.bukkit.command.Command;
import org.bukkit.command.CommandExecutor;
import org.bukkit.command.CommandSender;
import org.bukkit.entity.Player;
import org.jetbrains.annotations.NotNull;

public class HomeCommand implements CommandExecutor {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if(sender instanceof Player){
            Player p = ((Player) sender).getPlayer();

            if(p.hasPermission(HomePermissions.TELEPORT_PERMISSION)){ //Check if Player has permissions to teleport
                if(args.length == 1){

                    HomeSystemDatabase hsdb = new HomeSystemDatabase();
                    Home home = hsdb.getHome(p.getUniqueId(), args[0]);

                    if(home != null){
                        p.teleport(new Location(home.getWorld(), home.getX(), home.getY(), home.getZ(), home.getYaw(), home.getPitch()));
                        p.sendMessage(ChatColor.GREEN + HomeMessages.TELEPORT.replace("$home$", args[0]));
                    }else{//Home null = no home found with name
                        p.sendMessage(ChatColor.RED + HomeMessages.NO_HOME_FOUND.replace("$home$", args[0]));
                    }
                    return true;

                }
            }else{ //No permission
                sender.sendMessage(ChatColor.RED + HomeMessages.NO_PERMISSION);
                return true;
            }

        }else{ //Sender != Player
            sender.sendMessage("Dieser Befehl muss von einem Spieler ausgeführt werden!");
            return true;
        }


        return false;
    }
}
